...
- The repository exposes generic signing helpers for raw message bytes. These are treated as non-core utilities under `keys` (Experimental), and the `catf`-package exports are deprecated to avoid implying protocol-level bypass paths.
