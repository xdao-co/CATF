...
### Status: **Partially satisfied** (one correctness/safety issue)
...
**Open issue (library safety)**
- `crof.Render` currently *panics* if signing is requested and signing fails (`panic("crof: signing requested but failed: ...")` in `src/crof/crof.go`).
  - This violates “safe to wrap in a service/API without semantic change” and should be replaced with an error-returning API path.
...
**Open issues (strict = no defaults, no assumptions)**
- `crof.Render` applies an implicit default `Resolver-ID` when `opts.ResolverID == ""`.
  - `RenderWithCompliance(..., Strict)` does not currently reject missing `ResolverID`, so strict mode still permits this default.
- `tpdl.Parse` defaults `Require.Quorum` to `1` when not present.
  - If “strict mode means no defaults” is interpreted literally for policy parsing, there is no `tpdl` compliance switch today.
...
**Open issues (Go export surface vs documented tiers)**
- Several exported symbols are not explicitly listed in `STABILITY.md` under Stable/Experimental, which makes the effective API surface ambiguous for downstream users.
  - Example: `catf.Document`, `catf.Render`, signing helpers in `catf`, etc. (these are exported and therefore part of the Go public API whether intended or not).
- CROF signing behavior currently panics on error (also impacts API stability expectations for “safe embedding”).
...
1) Address CROF signing panic by providing an error-returning signing render path.
2) Tighten strict mode to eliminate defaults (notably CROF `Resolver-ID` defaulting) when `compliance.Strict` is selected.
3) Decide and implement how strict mode should apply (or not apply) to TPDL quorum defaults.
4) Close remaining negative/adversarial tests identified above.
5) Reconcile `STABILITY.md` with the actual exported Go API surface (document or re-tier symbols explicitly).
