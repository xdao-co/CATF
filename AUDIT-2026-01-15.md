...
- The repository exposes generic signing helpers for raw message bytes. These are now treated as non-core utilities under `keys` (Experimental), and the `catf`-package exports are deprecated to avoid implying protocol-level bypass paths.
...
- CROF byte/structure malformations (BOM/invalid UTF-8, missing pre/postamble, unexpected content, missing/out-of-order sections, unsorted META, missing required META keys, missing section terminator blank line, malformed RESULT/PATHS/FORKS/EXCLUSIONS/VERDICTS/CRYPTO records; RESULT now rejects unknown keys and validates Policy-* schemas): `src/crof/canonicalize_test.go`.
...
- (Done) Conflicting supersession heads surface a stable fork output: `TestResolve_SupersedesCompetingHeadsSurfaceFork` (`src/resolver/resolver_test.go`).
- (Done) Structural/byte-level malformed CROF coverage is now pinned in `src/crof/canonicalize_test.go` (sections, ordering, required keys, record structure, CRYPTO, and RESULT Policy-* schemas).
...
## Verification Checklist (quick re-validation)

Run from repo root:

- `make test`
- `make regen-conformance && make test`

Expected invariants:

- CROF canonicalization is reject-only and comprehensive (including RESULT Policy-* schemas) via `crof.CanonicalizeCROF`.
- Strict resolver modes reject ambiguous outputs and policies missing explicit quorum.
- Determinism holds under shuffled inputs (resolver + CROF bytes).
